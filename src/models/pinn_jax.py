...
if __name__ == "__main__":
    ...
    key, subkey = random.split(key)
    data = {
        't_data': random.uniform(subkey, (n_data,), minval=0, maxval=1),
        'x_data': random.uniform(subkey, (n_data,), minval=-1, maxval=1),
        'y_data': random.uniform(subkey, (n_data,), minval=-1, maxval=1),
        'z_data': random.uniform(subkey, (n_data,), minval=-1, maxval=1),
        'u_data': random.normal(subkey, (n_data,))
    }
    
    # Train
    trained_model = trainer.train(data, epochs=100)
    
    # Update with symbolic equation
    new_equation = "u_tt - 1.0 * u_xx - 0.1 * sin(2 * pi * x)"
    equation_fn = compile_symbolic_to_jax(new_equation)
    trainer.set_symbolic_equation(equation_fn)
